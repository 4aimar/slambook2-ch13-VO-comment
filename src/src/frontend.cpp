...
namespace myslam
{
    ...
    added by caohm
    1. 传统计算机视觉中的特征点
    在传统计算机视觉中，特征点通常使用 cv::KeyPoint 类来表征，特征点检测算法（如 SIFT、SURF、ORB 等）会检测到图像中的特征点并使用该类存储其信息。cv::KeyPoint 类主要存储以下内容：

    位置（pt）：特征点在图像中的二维坐标 (x, y)。
    尺寸（size）：特征点的大小，表示检测到特征点的区域范围。
    方向（angle）：特征点的方向，通常是特征点周围区域的主要方向。
    响应值（response）：特征点的强度或显著性，通常是特征检测算法的响应值。
    层级（octave）：特征点所在的金字塔层级，用于多尺度特征检测。
    分类ID（class_id）：特征点的分类ID，可用于聚类或分类任务。
    这些信息主要用于描述特征点的位置、大小和方向，以便进行图像匹配、跟踪和三维重建等任务。

    2. 深度学习中的特征
    在深度学习中，特征通常是通过卷积神经网络（CNN）从图像中提取的高维特征向量。这些特征向量的内容和长度取决于网络的架构和配置，通常包括以下特点：

    高维向量：特征向量通常是高维的（例如 1024 维、2048 维等），每个维度都是一个浮点数，表示图像或图像区域的某个特征。
    全局特征和局部特征：卷积神经网络可以提取图像的全局特征（如整体形状）和局部特征（如纹理、边缘）。
    不可解释性：这些特征向量的每个维度通常没有具体的物理意义，它们是网络自动学习到的用于区分不同类别或模式的特征。
    */
    int Frontend::DetectFeatures()
    {
        /*
        added by caohm
        利用了OpenCV的Good Features to Track (GFTT) 算法来检测特征点
        提取特征点的常用算法有很多Harris角点检测,ORB (Oriented FAST and Rotated BRIEF),FAST (Features from Accelerated Segment Test),深度学习特征提取
        传统特征检测算法在资源受限的环境中具有优势，适用于实时应用。深度学习特征提取方法则具有更强的鲁棒性和适应性，但需要较大的计算资源和数据支持。选择使用哪种方法取决于具体应用场景的需求和资源限制。
        */
    ...
    int Frontend::FindFeaturesInRight()
    {
        ...
        for (auto &kp : current_frame_->features_left_)
        {
            /*
            added by caohm
            将当前特征点的图像坐标位置 kp->position_.pt 添加到 kps_left 向量中。kps_left 存储了所有左目图像中特征点的位置。
            */
            ...
            { // 如果赋予成功，则mp是一个非空的智能指针，则执行if语段
                ...
                /*
                added by caohm
                检查是否成功获取到 MapPoint，如果有的话
                使用右目相机模型 camera_right_ 将该 MapPoint 的三维世界坐标 mp->pos_ 投影到当前帧的右目图像上。
                camera_right_->world2pixel 函数将三维世界坐标转换为右目图像的像素坐标。
                */
                /*
                added by caohm
                假设有两个相机，左目和右目，位置不同，但都观察到了同一个三维点 MapPoint。这个点的三维坐标是 mp->pos_。
                camera_left_->world2pixel(mp->pos_, current_frame_->Pose()) 计算的是左目相机将这个三维点投影到左目图像上的像素坐标。
                camera_right_->world2pixel(mp->pos_, current_frame_->Pose()) 计算的是右目相机将这个三维点投影到右目图像上的像素坐标。
                由于左目和右目相机的位置和方向不同，同一个三维点在左目和右目图像上的像素坐标也会不同。因此，当有 MapPoint 存在时，kps_right 中的值会与 kps_left 不同，这反映了同一个三维点在两个不同相机视角下的投影差异。
                */
                /*
                added by caohm
                在初始化阶段：
                左目图像的特征点通常还没有对应的 MapPoint，即 kp->map_point_ 为空。
                调用 FindFeaturesInRight 函数：

                遍历左目图像中的特征点，对于每个特征点，尝试获取 MapPoint。
                因为此时大多数特征点没有 MapPoint，所以 mp 为空。
                执行代码分支：

                if (mp) 这个分支通常不会被执行，而是执行 else 分支，使用左目图像中的特征点位置作为右目图像中特征点的初始估计位置。
                光流跟踪：

                cv::calcOpticalFlowPyrLK 使用这些初始估计位置进行光流跟踪，找到右目图像中对应的特征点位置。
                */
                auto px =
            ...
            { // 如果指针为空则执行else语段
                ...
                added by caohm
                如果没有成功获取到 MapPoint
                直接将左目图像中的特征点位置 kp->position_.pt 作为右目图像中特征点的初始估计位置，添加到 kps_right 向量中。
                */
        ...
        /*
        added by caohm
        如果没有 MapPoint，右目摄像头中的特征点位置可能不准确。这种情况下，使用光流跟踪可能会受到影响，但这也是光流跟踪算法的一部分设计：通过逐步迭代找到精确的匹配点。
        光流跟踪算法（如 calcOpticalFlowPyrLK）的工作原理是基于图像亮度不变假设，寻找特征点在相邻帧或相邻视角中的对应点。
        即使初始估计位置不准确，光流跟踪算法也有一定的鲁棒性，可以在一定范围内找到正确的特征点位置。
        */
        /*
        added by caohm
        这行代码使用了 OpenCV 的 calcOpticalFlowPyrLK 函数来进行光流跟踪。参数如下：
            current_frame_->left_img_：左目图像。
            current_frame_->right_img_：右目图像。
            kps_left：左目图像中的特征点位置。
            kps_right：右目图像中的特征点初始估计位置。
            status：输出的状态向量，表示每个特征点是否跟踪成功。成功为 1，失败为 0。
            error：输出的误差向量。
            cv::Size(11, 11)：搜索窗口的大小。
            3：金字塔层数。
            cv::TermCriteria：终止条件。
    ...
    通常我们用sizeof(XXX)操作，这个操作所得到的结果就是size_t类型。因为size_t类型的数据其实是保存了一个整数，
            ...
            if (status[i])
            {
                /*
                added by caohm
                kps_left[i] 和 kps_right[i] 是初始计算好的。如果左目特征点有 MapPoint，那么 kps_right[i] 是通过三维投影得到的右目图像中的位置，是准确的；如果没有 MapPoint，kps_right[i] 的初始值是 kps_left[i]，也就是相同的位置。
                当调用光流跟踪函数时，如果 status[i] 表示跟踪成功，那么 kps_right[i] 的位置实际上是通过光流跟踪算法在右目图像中找到的匹配位置，不再是初始的 kps_left[i] 的位置。
                光流跟踪函数在 kps_right[i] 的初始估计位置附近搜索，并找到最佳匹配位置，因此可以直接使用更新后的 kps_right[i] 作为右目图像中特征点的位置，添加到 current_frame_->features_right_ 中。
                关于 cv::KeyPoint 构造函数中的 7，它表示的是特征点的直径。特征点的直径是用来描述特征点的大小或区域范围，这在某些算法中可能用来加权计算或者滤波处理。
                */
    ...
    bool Frontend::BuildInitMap()
    {
        /*
        added by caohm
        camera_left_->pose() 和 camera_right_->pose()这两个摄像头是在读配置文件时，已经通过投影矩阵得到了旋转矩阵+平移矩阵，最终构造了SE3作为相机的位姿pos传递过来了
        */
        ...
        /*
        added by caohm
        每一帧中有很多特征点，都存放在了map_中，map_->InsertMapPoint,同样如果这一帧是一个关键帧，也需要map_->InsertKeyFrame(current_frame_);后端还会对关键帧进行优化backend_->UpdateMap();
        */
        // 遍历左目
        for (size_t i = 0; i < current_frame_->features_left_.size(); ++i)
        {
            ...
            if (triangulation(poses, points, pworld) && pworld[2] > 0)
            {
                ...
                */
                ...
                /*
                added by caohm
                在初始化时，前端已经和map相关联，所以现在可以通过前端中的指向map的智能指针map_对map上的MapPoint和KeyFrame进行操作
                */
                /*
                added by caohm
                每一个MapPoint在构造时都有一个唯一id，前端将这个Frame中的所有特征的MapPoint都插入到map_中，并且将当前Frame作为key frame插入到map_里
                在map中有两个变量
                ...
                这里前端每次插入一个新的MapPoint都把它当成一个active_landmakrs了
                */
        ...
        /*
        added by caohm
        每一个关键帧都插入到map的keyframes_里了，但并不是把每一帧都插入到active_keyframes_, 有一个最大窗口num_active_keyframes_，超过最大窗口调用RemoveOldKeyframe
        这一点不同于MapPoint的active_landmarks_
        */
